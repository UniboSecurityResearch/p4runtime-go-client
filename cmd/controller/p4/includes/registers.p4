...
register<bit<1>>(1) block_writing; // to prevent changes in the above registers during the scraping with switch API
...
register<bit<100>>(1) not_foldable_counter0;//just for test
register<bit<100>>(1) not_foldable_counter1;//just for test


register<bit<48>>(1) last_seen;//window time 
