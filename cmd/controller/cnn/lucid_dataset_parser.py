...
DDOS_FLOWS_CUSTOM = {'attackers': ['192.168.8.1','192.168.8.2','192.168.8.3','192.168.8.4','192.168.8.5','192.168.8.6','192.168.8.7','192.168.8.8','192.168.8.9','192.168.8.10','192.168.8.11','192.168.8.12','192.168.8.13','192.168.8.14','192.168.8.15',
                                   '172.50.45.1','172.50.45.2','172.50.45.3','172.50.45.4','172.50.45.5','172.50.45.6','172.50.45.7','172.50.45.8','172.50.45.9','172.50.45.10','172.50.45.11','172.50.45.12','172.50.45.13','172.50.45.14','172.50.45.15',
                                   '205.101.50.1','205.101.50.2','205.101.50.3','205.101.50.4','205.101.50.5','205.101.50.6','205.101.50.7','205.101.50.8','205.101.50.9','205.101.50.10','205.101.50.11','205.101.50.12','205.101.50.13','205.101.50.14','205.101.50.15',
                                   '100.215.1.1','100.215.1.2','100.215.1.3','100.215.1.4','100.215.1.5','100.215.1.6','100.215.1.7','100.215.1.8','100.215.1.9','100.215.1.10','100.215.1.11','100.215.1.12','100.215.1.13','100.215.1.14','100.215.1.15'],
                      'victims': ['10.0.2.1']}
...
DDOS_ATTACK_SPECS = {
    ...
    'CUSTOM_HPING_DS' : DDOS_FLOWS_CUSTOM,
...
def parse_packet(pkt, p4_compatible=False):
    pf = packet_features()
    tmp_id = [0,0,0,0,0]
    ...
    try:
        ...
        if pkt.transport_layer != None:
            if protocol == socket.IPPROTO_TCP:
                ...
                pf.features_list.append(int(pkt.tcp.ack.raw_value,16)) # pf.features_list.append(int(pkt.tcp.ack))  # TCP ack
...
def process_pcap(pcap_file,dataset_type,in_labels,max_flow_len,labelled_flows, p4_compatible, traffic_type='all',time_window=TIME_WINDOW):
    ...
    for i, pkt in enumerate(cap):
        ...
        pf = parse_packet(pkt, p4_compatible)
...
def generate_and_store_packets_from_features(features,temp_dict,start_time_window,max_flow_len):
    """
    ...
    for index in range(0,number_of_packets):
        ...
        src_port = features[index][FEATURES_LIST[9]]
        dst_port = features[index][FEATURES_LIST[10]]
        src_ip = str(ipaddress.IPv4Address(features[index][FEATURES_LIST[11]]))
        dst_ip = str(ipaddress.IPv4Address(features[index][FEATURES_LIST[12]]))
...
def process_live_traffic(traffic_source, dataset_type, in_labels, max_flow_len, p4_compatible, filtered, traffic_type='all',time_window=TIME_WINDOW):
    start_time = time.time()
    temp_dict = OrderedDict()
    labelled_flows = []
    ...
    if isinstance(traffic_source, pyshark.LiveCapture) == True:
        for pkt in traffic_source.sniff_continuously():
            if time.time() >= time_window:
                break
            pf = parse_packet(pkt,p4_compatible)
            temp_dict = store_packet(pf, temp_dict, start_time_window, max_flow_len)
    elif isinstance(traffic_source, pyshark.FileCapture) == True:
        while time.time() < time_window:
            pkt = traffic_source.next()
            pf = parse_packet(pkt, p4_compatible)
            temp_dict = store_packet(pf,temp_dict,start_time_window,max_flow_len)
        ...
        features = requests.get(url="http://localhost:3333/getCollectedDigests").json()
        ...
        #else:
        total_incoming_packets=len(featuresfiltered)
        ...
        t0=time.time()
        temp_dict = generate_and_store_packets_from_features(featuresfiltered,temp_dict,start_time_window,max_flow_len)
        processing+=time.time()-t0


        flows=temp_dict.keys()
        dataset_info = DDOS_ATTACK_SPECS[dataset_type]

        packets_per_sample_sizes=dict()
        packets_per_sample_sizes["good"]=Counter()
        packets_per_sample_sizes["bad"]=Counter()

        for k in flows:
            if k[0] in dataset_info['attackers']:
                packets_per_sample_sizes["bad"][len(temp_dict[k][start_time_window])]+=1
            else:
                packets_per_sample_sizes["good"][len(temp_dict[k][start_time_window])]+=1

    apply_labels(temp_dict,labelled_flows, in_labels,traffic_type)

    #filteredReq = requests.get(url="http://localhost:10000/info/0").json()
    #if(filtered == 0):
    #    nbytes = filteredReq['StateSize']
    #if(filtered == 1):
    #    nbytes = filteredReq['StateSizeFiltered']
    #print(nbytes)

    nbytes="0.00"

    return labelled_flows, processing, extraction, packets_per_sample_sizes, packets_in_registers, total_incoming_packets, nbytes
...
def main(argv):
    ...
    parser.add_argument('--p4_compatible', action='store_true',
                        help='Use only p4 compatible features: remove Highest Layer and Protocols')
    ...
    if args.dataset_folder is not None and args.dataset_type is not None:
        ...
        for file in filelist:
            try:
                flows = manager.list()
                p = Process(target=process_pcap,args=(file,args.dataset_type[0],in_labels,max_flow_len,flows, args.p4_compatible, traffic_type,time_window))
        ...
        if args.p4_compatible:
            filename = str(int(time_window)) + 't-' + str(max_flow_len) + 'n-' + dataset_id + '-p4' + '-preprocess'
        else:
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        for file in filelist:
            ...
            p4_compatible = True if str(filename.split('-')[3].strip()) == "p4" else False
        ...
        if p4_compatible:
            output_file = output_folder + '/' + str(time_window) + 't-' + str(max_flow_len) + 'n-' + dataset_id + '-dataset-p4'
        else:
