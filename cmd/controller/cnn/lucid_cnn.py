...
PREDICTION_HEADER = "Model            TIME(sec) PACKETS SAMPLES DDOS% ACC    ERR    F1     PPV    TPR    FPR    TNR    FNR    extr(sec)    proc(sec)    registers_occupation    total_packet_captured    Data_Source    packets_per_sample_sizes \n"
...
def main(argv, tcpreplay_pid):
    ...
    parser.add_argument('-f', '--filtered', default=0, type=int,
                        help='Want all traffic or only filtered? f=0 to get all traffic, f=1 else')

    args = parser.parse_args()


    #print(type(args.filtered))
    ...
    if args.predict_live is not None:
        ...
        speed=str(os.getenv("attack_speed"))
        register_bits=str(os.getenv("register_bits"))
        register_width=str(os.getenv("register_width"))
        sampling=str(os.getenv("sampling"))
        stats_file = open('./log/predictions-' + time.strftime("%Y%m%d-%H%M%S") + '_' + speed + '_' + register_bits + '_' + register_width + '_' + sampling  + '.csv', 'a')
        stats_file.write(PREDICTION_HEADER)

        arch_file = open('./log/arch-'+ str(args.filtered) +'.csv', 'a')
        arch_file.write("PROCESS-TIME SIZE TIMESTAMP\n")
        ...
        while (True):
            ...
            samples, process_time, trasmission_time, packets_per_sample_sizes, avg_packets_in_registers_in_round, total_packet_captured_in_round, nbytes =  \
                process_live_traffic(traffic_source, args.dataset_type, labels, max_flow_len, p4_compatible, 
                args.filtered,traffic_type="all", time_window=time_window)
            if len(samples) > 0:
                ...
                for flow in flows_set.keys():
                    if(True in np.array(flows_set[flow]) and len(np.array(flows_set[flow])) > 10):
                        print("--------------NEW MEASUREMENT---------------")
                        print(str(flow) + "--> ")
                        rate = report_results_per_flow(Y_true, Y_pred, prediction_time + process_time, nbytes, None) #PREDICTION TOTAL
                        #da mettere
                        ...
                        requests.post(url="http://localhost:3333/updateDroppedFlow", json=payload)

                        if(rate >= 0.5):
                            requests.post(url="http://localhost:3333/dropFlow", json=payload)
                        else:
                            requests.post(url="http://localhost:3333/undropFlow", json=payload)    

                
                time.sleep(15)


def report_results_per_flow(Y_true, Y_pred,prediction_time, nbytes, arch_file):

    ddos_rate = 0
    
    ddos_rate = '{:04.3f}'.format(sum(Y_pred)/Y_pred.shape[0])
    time_string_predict = 'TIME: {:10.3f}'.format(prediction_time) + " "

    performance_string = 'DDOS %: {}'.format(ddos_rate) + " "

    size_string = 'SIZE (in bytes): '+ nbytes +" "
    ...
    if Y_true is not None: # if we have the labels, we can compute the classification accuracy
        ...
        output_string = time_string_predict + performance_string + size_string + test_string_pre

        if arch_file is not None:
            arch_file.write('{}'.format(prediction_time)+' {}'.format(nbytes)+' {}'.format(time.time()) +"\n")
            arch_file.flush()

        print(output_string)
        #print('{}'.format(prediction_time)+' {}'.format(nbytes)+' {}'.format(time.time()) +"\n")
...
def report_results(Y_true, Y_pred,packets, model_name, dataset_filename, stats_file,prediction_time,process_time, 
    ...
    if Y_true is not None: # if we have the labels, we can compute the classification accuracy
        ...
        test_string_pre = '{:05.4f}'.format(accuracy) + \
                          ...
                          " " + '{:05.4f}'.format(trasmission_time) + " "*7 + '{:05.4f}'.format(process_time) + \
                          " "*7 + '{:05.4f}'.format(avg_packets_in_registers_in_round) + " "*15 + '{:05.4f}'.format(total_packet_captured_in_round) + \
                          " "*15 + dataset_filename + \
                          " " + json.dumps(packets_per_sample_sizes).replace(" ","") + "\n"
        output_header = PREDICTION_HEADER[:-1]
